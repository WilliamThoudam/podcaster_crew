...
def planning_analyst_system_prompt() -> str:
    return (
        ...
        "- When the user asks about forecasts, projections, future trends, or outlook, ensure at least one "
        "sub_question retrieves historical time-series data (e.g., annual or monthly totals by the requested "
        "dimensions over the last 3-5 years with the time column preserved) so the FORECASTER agent can "
        "extrapolate trends. Do NOT collapse time into averages for forecast-intent questions.\n"
        "- For forecast-intent questions that slice by **country and product category** (or similar "
        "high-cardinality dimensions), **bound the result size** so text-to-SQL can return a useful time "
        "series in the first rows: e.g. restrict to **top 10–15 countries by total sales** and/or **top 8–12 "
        "product categories**, and ask for **annual (or monthly) totals by year (or month) ordered by time "
        "ascending** (ORDER BY year, then country, then category). Avoid one unconstrained global query that "
        "returns thousands of rows where the model only sees the first slice of rows (same year / one country).\n"
